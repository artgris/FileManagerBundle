        ...
        function initTree(treedata) {
            $('#tree').jstree({
                    ...
                    items: function ($node) {
                        return {
                            "create_folder": {
                                ...
                                "label": newFolderMessage,
                                "action": function (data) {
                                    $('#addFolder').modal('show');
                                    var $a_href = data.reference[0].getAttribute("href");
                                    $a_href_folder = new URLSearchParams($a_href.split('?')[1]).get('route');
                                    document.getElementById('rename_defined_parent_path').value = $a_href_folder;
                                }
                            },
                            "delete_folder": {
                                ...
                                "label": deleteMessage,
                                "action": function (data) {
                                    document.querySelector('.js-delete-modal').click();
                                    var $a_href = data.reference[0].getAttribute("href");
                                    $a_href_folder = new URLSearchParams($a_href.split('?')[1]).get('route');
                                    document.getElementById('delete_f_defined_parent_path').value = $a_href_folder;
                                }
                            },
                            "rename_folder": {
                                "label": renameMessage,
                                "action": function (data) {
                                    $('#js-confirm-folder-rename').modal('show');
                                    var $old_name = data.reference[0].innerText;
                                    document.getElementById('rename_folder_name').value = $old_name;
                                    document.getElementById('rename_folder_oldname').value = $old_name;
                                    var $a_href = data.reference[0].getAttribute("href");
                                    $a_href_folder = new URLSearchParams($a_href.split('?')[1]).get('route');
                                    document.getElementById('rename_folder_oldpath').value = $a_href_folder;
                                }
                            },
                        }
                    },
                ...
                if (str.nodes.length === 0) {
                    $('#jstree-folder-search').jstree(true).hide_all();
                }
            });
        }
